...
import spDataJson from "./initData.json";
import "./app.css";
...
class Main extends React.Component {
  ...
  async catItemActions(file, url, op) {
    try {
      if (fileImg !== null) {
        const res = await fetchDelPHP(tempIcon, "./api/handler.php", url);
        // console.log("Seems deleted!");
        // console.log("Delete result=", res);
      }
      if (fileCatImg !== null) {
        const resCat = await fetchDelPHP(tempCatIcon, "./api/handler.php", url);
        // console.log("Seems deleted!");
        // console.log("Delete result=", res);
      }
      tempIcon = "";
      ...
      const nome = await fetchUpPHP(file, "./api/handler.php", url);
      ...
      this.saveConf(spData, "./api/handler.php", "config");
      fileCatImg = null;
      fileImg = null;
    } catch (err) {
      console.error(err);
  ...
  applyItemDel = () => {
    ...
    if (index !== -1) {
      fetchDelPHP(tempIcon, "./api/handler.php", "icon");
      tempIcon = "";
  ...
  loginEditCheck = async () => {
    if (this.changeFlagRef.current) {
      ...
      try {
        // If the user fills the user/psw fields
        if (usrTmp !== "" || pswTmp !== "") {
          const data = await updateCredentials(usrTmp, pswTmp, "./api/handler.php");
        ...
        // Always save config (colors, etc.)
        await fetchUpConfig(spData, "./api/handler.php", "config");
        this.fireAlert("Changes Made!", "solidgreen");
              ...
              <div className="modal-content noborder">
                ...
                <div className="modal-body">
                  <form id="backEditForm">
                    ...
                    {/* CAT BACK OPACITY */}
                    <InputOpacity opacity={spData.catOpacity} id="catOpRange" tempo={e => tempOpacity1 = e.target.value} changeFlagRef={this.changeFlagRef} dialog="main"></InputOpacity>
                    {/* CAT BEFORE ITEMS */}
                ...
                <div className="modal-body align-items-center darkBG">
                  <center>
                    <iframe id="myvideo" className="iframeStyle"
                      src={this.state.videoLink || null} style={{ border: 0 }}
