...
export async function fetchAuth(url, data) {
    const response = await fetch(url, {
        method: 'POST',
        body: data,
        credentials: 'include', // Mantiene la sessione PHP attiva
        headers: { 'Accept': 'application/json' }
    });
    if (!response.ok) throw new Error("Errore di rete o sessione scaduta");
...
export async function fetchUpPHP(file, url, key) {
    ...
    const json = await fetchAuth(url, data);
    return json.filename; // Restituisce il nome file per usarlo nel componente
}
...
// Nuova funzione specifica per le credenziali
export async function updateCredentials(user, pass, url) {
    const data = new FormData();
    data.append('update_credentials', 'true');
    data.append('new_user', user);
    data.append('new_pass', pass);
    return await fetchAuth(url, data);
}
...
// e
// xport function FormChanges(form) {
//     let currValue;
//     // get form
